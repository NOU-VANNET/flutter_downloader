...
class FlutterDownloaderPlugin : MethodChannel.MethodCallHandler, FlutterPlugin {
    ...
    private fun enqueue(call: MethodCall, result: MethodChannel.Result) {
        ...
        val metaData: String? = call.argument("meta_data")
    ...
    private fun loadTasks(result: MethodChannel.Result) {
        ...
        for (task in tasks) {
            ...
            item["meta_data"] = task.metaData
            array.add(item)
        }
        result.success(array)
    }

    private fun loadTask(call: MethodCall, result: MethodChannel.Result) {
    ...
    private fun updateMetaData(call: MethodCall, result: MethodChannel.Result) {
        val taskId: String = call.requireArgument("task_id")
        val metaData: String = call.requireArgument("meta_data")
        taskDao!!.updateMetaData(taskId, metaData)
        result.success(true)
    }
