...
class TaskDao(private val dbHelper: TaskDbHelper) {
    ...
    fun insertOrUpdateNewTask(
        ...
        values.put(TaskEntry.COLUMN_META_DATA, metaData ?: "")
        db.beginTransaction()
        try {
    ...
    fun updateMetaData(taskId: String, metadata: String) {
        val db = dbHelper.writableDatabase
        val values = ContentValues()
        values.put(TaskEntry.COLUMN_META_DATA, metadata)
        db.beginTransaction()
    ...
    private fun parseCursor(cursor: Cursor): DownloadTask {
        ...
        val metaData = cursor.getString(cursor.getColumnIndexOrThrow(TaskEntry.COLUMN_META_DATA))
        return DownloadTask(
            primaryId,
            taskId,
